    ...
    <form class="form primary" v-on:submit.prevent="onSubmit">
      ...
      <div class="form">
        <div class="form-group col-md-6">
          ...
          <v-select :options="kingdoms" label="KingdomName" v-model="event.kingdom" @select="getParks"></v-select>
          <div class="help-block">
          </div>
        </div>
        <div class="form-group col-md-6">
          <label for="Park">Park</label>
          <v-select :options="parks" v-model="event.park" label="Name"></v-select>
        </div>
        <div class="form-group col-md-6">
          <label>Name</label>
        ...
        <div class="form-group col-md-6">
          ...
          <select v-model="event.EventId" class="form-control">
            <option></option>
            <option v-for="template in templates" :value="template.EventId">{{ template.Name }}</option>
          </select>
        </div>
        <div class="form-group" v-if="event._id">
          <label>OrkId</label>
          <input type="text" v-model="event.EventCalendarDetailId" class="form-control">
        </div>
        ...
        <div class="form-group col-md-4">
          <label>Days</label>
          <input type="number" min="1" max="12" v-model="event.days" class="form-control">
        </div>
        <div class="form-group col-md-4">
          <label>Credits/Day</label>
          <input type="number" min="1" max="12" v-model="event.credits" class="form-control">
        </div>
        <div class="form-group col-md-4">
          <label>Fee</label>
          <div class="input-group">
            <span class="input-group-addon">$</span>
            <input type="number" min="1" max="120" v-model="event.fee" class="form-control">
          </div>
        </div>
        <div class="form-group col-md-8">
          ...
          <select v-model="event.location" class="form-control">
            <option></option>
              <option v-for="loc in locations" :value="loc._id">{{ loc.name }}</option>
          </select>
        </div>
        <div class="row">
          <div class="col-md-6 form-group">
            <label>Registration Emails</label>
            <div class="help-block">Emails that pre-registrations will be sent to</div>
            <div v-for="email, i in event.registrationEmails" :key="i" class="input-group">
              <input type="email" required v-model="event.registrationEmails[i]" class="form-control" />
              <span class="input-group-addon">
                <button v-if="i > 0" type="button" class="btn btn-xs input-group-btn" @click.prevent="removeEmail(event.registrationEmails, i)">
                  <span class="glyphicon glyphicon-minus-sign"></span>
                </button>
              </span>
            </div>
            <div>
              <button class="btn btn-md" @click.prevent="addEmail(event.registrationEmails)">
          ...
          <div class="col-md-6 form-group">
            ...
            <div class="help-block">Emails that volunteer remquests will be sent to</div>
            <div v-for="email, i in event.volunteerEmails" :key="i" class="input-group">
              <input type="email" required v-model="event.volunteerEmails[i]" class="form-control" />
              <span class="input-group-addon">
                <button v-if="i > 0" type="button" class="btn btn-xs input-group-btn" @click.prevent="removeEmail(event.volunteerEmails, i)">
                  <span class="glyphicon glyphicon-minus-sign"></span>
                </button>
              </span>
            </div>
            <div>
              <button class="btn btn-md" @click.prevent="addEmail(event.volunteerEmails)">
                <span class="glyphicon glyphicon-plus-sign"></span>
              </button>
            </div>
          </div>
      ...
      <div class="row">
        ...
        <span class="glyphicon glyphicon-plus-sign" @click="addCratPosition"></span>
        <span>{{ event.crats.length }}</span>
        <div v-for="crat, i in event.crats" :key="crat.role" class="form-group col-md-4">
          <input v-model="crat.role" type="text" class="form-control" placeholder="Crat Role"/>
          <input type="text" placeholder="Player" v-model="crat.playerName"/>
          <div v-if="crat.options.length > 0" class="list-group">
            <Player class="list-group-item" v-for="player in players" :key="player.MundaneId" :player="player" @click="setPlayer(crat, player, i)" />
          </div>
        </div>
      </div>
      ...
      <div class="btn-group">
        <button type="submit" class="btn btn-primary">{{ (event._id) ? 'Submit' : 'Add Event' }}</button>
        <button type="button" v-if="event._id" class="btn btn-default" @click="cancelEdit">Cancel</button>
        <button type="button" v-if="event._id" class="btn btn-danger" @click="deleteEvent(event)">Delete</button>
      </div>
    </form>
  </div>
</template>
