<template>
  <div>
    <div class="container playerData">
      ...
      <div class="avatar" v-if="playerDetails.HasImage || playerDetails.HasHeraldry" &#x3C;div>
        <img
          :src="'https:' + playerDetails.Image"
      ...
      <div class="avatar">
        <img
          :src="'https:' + playerDetails.Heraldry"
      ...
      <table class="table table-striped">
        <tbody>
          ...
          <tr>
            ...
            <th>Email</th>
            <td>
              <a :href="'mailto:' + playerDetails.Email">{{ playerDetails.Email }}</a>
            </td>
          </tr>
          <tr>
            <th>Kingdom</th>
            <td>
              <nuxt-link
                :to="{ name: 'Parks', params: { kingdomId: playerDetails.KingdomId } }"
              >{{ playerDetails.KingdomName }}</nuxt-link>
            </td>
            <th>Park</th>
            <td>
              <nuxt-link
                :to="{ name: 'Park', params: { parkId: playerDetails.ParkId } }"
              >{{ playerDetails.ParkName }}</nuxt-link>
            </td>
          </tr>
          <tr v-if="playerDetails.Notes && playerDetails.Notes.length > 0">
            <td colspan="4">
              ...
              <ul :class="notes">
                <li v-for="note in playerDetails.Notes" :key="note.id">
                  <div>
