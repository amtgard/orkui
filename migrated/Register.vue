<template lang="html">
    ...
    <div class="form-container">
      <form @submit.prevent="onSubmit">
        <div class="heading">
          ...
          <p class="secondary">
            {{ event.date }} {{ (location) ? 'at ' + location.name : '' }}
          </p>
          <p class="text-muted">
            {{ event.description }}
          </p>
          ...
          <div class="column">
            <div class="form-group row">
              ...
              <div class="help-block">
                ...
                <p class="small">
                </p><form action="https://www.paypal.com/cgi-bin/webscr" target="_blank" method="post" class="pull-right">
            ...
            <div v-if="cabins.length > 0" class="form-group row">
              ...
              <select class="form-control" v-model="registration.cabin" required>
                <option v-for="c in cabins" :value="c._id">
                  {{ c.name }}
                </option>
              </select>
              <div class="help_block text-danger">
                {{ cabin_block }}
              </div>
            </div>
            <div class="form-group row">
              ...
              <div class="help-block">
                Please list and special needs and additional information here
              </div>
            </div>
          </div>
        </div>
        <div class="form-group">
          ...
          <button type="reset" @click="onClear" class="btn btn-warning">
            Reset
          </button>
        </div>
      </form>
  ...
  <div v-else>
    Please select an event <p class="text-muted">
      {{ $route.params.eventId }}
    </p>
    {{ (event) ? event.name : '' }}
    <button class="btn btn-primary" @click="fetchEvent">
      wtf
    </button>
  </div>
</template>
